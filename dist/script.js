...
console.log("🚀...liftoff!!");
  ...
  // -------------------Fetch Navigation Data-------------------
    ...
    .then(res => {
        if(res.status !== 200) {
            console.error('Looks like the data could not be fetched!!: ', res.statusText);
        } else {
            res.json()
            .then(resData => {
                ...
                html = `
                    <ul id="nav-list" class="nav-list">
                        <li class="nav-list__items">
                            <a id='${ firstCity.section }' class="nav-list__items-link" href="#" tabindex="1">${ firstCity.label }</a>
                        </li>
                        

                        ${ 
                            allOtherCities.map(city => `
                                <li class="nav-list__items">
                ...
                let activeItemWidth = Math.round(allNavAnchors[0].getBoundingClientRect().width).toString();
                ...
                allNavAnchors.forEach(el => {
                    ...
                    el.addEventListener('click', function handleClick () {
                        ...
                        let current = document.getElementsByClassName('active');
                            ...
                            activeItemWidth = Math.round(clickedElement.getBoundingClientRect().width).toString();
                ...
                const ResponsivePosition = () => {
                    allNavAnchors.forEach(el => {
                        ...
                        if (el.classList.contains('active')) {
                            activeItemWidth = Math.round(el.getBoundingClientRect().width).toString() + 'px';
